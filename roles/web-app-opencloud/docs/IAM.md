...
```bash
make exec INFINITO_CMD="container exec opencloud sh -c 'curl -fsS {{ OIDC.CLIENT.DISCOVERY_DOCUMENT }} | head -c 200'"
```
...
```bash
make exec INFINITO_CMD="container exec opencloud opencloud config get ldap"
```
