...
//   2. "nextcloud login" — complete the OIDC handoff via Keycloak,
...
test("nextcloud login", async ({ page }) => {
  const expectedNextcloudBaseUrl = nextcloudBaseUrl.replace(/\/$/, "");

  const usernameField = page.getByRole("textbox", { name: nextcloudUsernameFieldPattern });
  const passwordField = page.getByRole("textbox", { name: "Password" });
  const rememberMeCheckbox = page.getByRole("checkbox", { name: "Remember me" });
  const signInButton = page.getByRole("button", { name: nextcloudCredentialSubmitPattern });
  const userMenuTriggerInMount = page.locator("#user-menu button");
  const logoutLinkByName = page.getByRole("link", { name: "Log out" });
  const logoutLinkByHref = page.locator('a[href*="logout"]');
  const logoutConfirmButton = page.getByRole("button", { name: "Logout" });
  const userMenuCandidates = [
    { kind: "user-menu", locator: userMenuTriggerInMount }
  ];
  const postLoginCandidates = [
    ...userMenuCandidates,
    ...getNextcloudShellCandidates(page)
  ];

  // Navigate directly to Nextcloud login
  await page.goto(`${expectedNextcloudBaseUrl}/login`, {
    waitUntil: "domcontentloaded",
    timeout: 60_000
  }).catch(() => {});
  ...
  if (await rememberMeCheckbox.first().isVisible().catch(() => false)) {
    ...
    await page.getByText("Remember me").click({ timeout: 2_000 }).catch(() => {});
  }
  ...
  await expect
    .poll(() => page.url(), {
      timeout: 60_000,
      message: `Expected redirect back to Nextcloud after Keycloak login: ${expectedNextcloudBaseUrl}`
    })
  ...
  await dismissBlockingNextcloudModals(page, page);
  ...
  await clickUserMenuWithModalRetry(page, page, userMenuState.locator);
