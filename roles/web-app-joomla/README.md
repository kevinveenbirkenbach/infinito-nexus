...
- the admin scenarios stay gated on `oidc` (and on `ldap` for the LDAP-variant scenarios), so a deploy with `SERVICES_DISABLED=oidc` reports the SSO scenario as `skipped`, never `failed`.
