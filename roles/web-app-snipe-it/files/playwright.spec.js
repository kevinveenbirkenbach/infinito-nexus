...
test("Snipe-IT front page is served under canonical domain with TLS", async ({ page }) => {
  const response = await page.goto(`${appBaseUrl}/`);
  expect(response, "Expected Snipe-IT response").toBeTruthy();
  expect(response.status(), "Expected Snipe-IT front page status < 400").toBeLessThan(400);
  expect(
...
test("Snipe-IT returns HTML content under canonical domain", async ({ request }) => {
...
test("administrator: dashboard → prometheus → app → universal logout", async ({ page }) => {
  await runAdminFlow(page, {
    adminInteraction: async (interactivePage) => {
      // web-app-snipe-it admin-only interaction: open a management surface.
