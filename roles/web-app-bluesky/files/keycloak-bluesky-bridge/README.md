# Keycloak to Bluesky PDS bridge SPI

A minimal Keycloak event-listener SPI that auto-provisions a Bluesky
PDS account on `REGISTER` and `LOGIN` events for users in the
configured target group, and stores the synthesised app-password
back on the Keycloak user as a custom attribute
(`bluesky_app_password`).

The user pastes that password into the official Bluesky web or app
...
per-role notes), this is the documented OIDC and LDAP integration
path for Bluesky. The RBAC integration is not feasible because the
PDS has no in-app role concept beyond "account exists / does not
exist".
...
|--------------------------------|--------------------------------------------------------------------|-------------------------------|
| `BLUESKY_PDS_URL`              | Base URL of the PDS, e.g. `https://pds.example`.                   | empty value, listener no-ops  |
...
1. In the realm Events tab, enable `bluesky-bridge` under
   "Event Listeners".
2. Create or verify the `/roles/web-app-bluesky` group exists.
...
* The bridge does NOT delete PDS accounts when the Keycloak user
  is removed. That is a separate cleanup concern out of scope for
  this requirement.
