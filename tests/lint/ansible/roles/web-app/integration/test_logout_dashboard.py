...
``# noqa: logout`` / ``# noqa: dashboard`` marker (or ``# nocheck:``)
on the line directly above the service key, paired with explicit
...
def _has_opt_out(config_path: Path, key: str) -> bool:
    ...
    3. ``# noqa: <key>`` (or ``# nocheck: <key>``) on the nearest
       non-empty line directly above the ``<key>:`` line in the raw
       YAML source.
    """
    if not config_path.is_file():
        return False
...
class TestWebAppLogoutDashboardIntegration(unittest.TestCase):
    ...
    OR carry an explicit ``# noqa: <key>`` opt-out paired with both
    flags ``false``.
    """
    ...
    def _check_service_activated(self, role_path: Path, key: str) -> str | None:
        ...
        rel = (
            ...
            if config.is_file()
            else role_path.relative_to(repo_root()).as_posix()
        )
        missing: list[str] = []
        ...
        return (
            ...
            f"with a ``# noqa: {key}`` (or ``# nocheck: {key}``) comment "
            f"on the line directly above ``{key}:`` and set both "
            f"flags to ``false``."
        )
