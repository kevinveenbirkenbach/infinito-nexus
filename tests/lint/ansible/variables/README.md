...
- `test_dotted_path_keys_exist.py` validates that every Jinja dotted path (`FOO.BAR.BAZ`) whose head is in `group_vars/all/` or the consuming role's own `vars`/`defaults` resolves to an existing key in that dict tree — catches typos / renamed / removed sub-keys. Per-line suppression: `# nocheck: dotted-path`.
