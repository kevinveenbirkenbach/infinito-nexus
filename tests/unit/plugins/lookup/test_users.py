...
class TestUsersLookup(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._cwd = os.getcwd()
        self._tmp = Path(self._cwd) / ".tmp_test_lookup_users"
        if self._tmp.exists():
    ...
    def test_hydrates_tokens_from_default_store_path_when_path_vars_missing(
        ...
        previous_default = runtime_lookup_data.DEFAULT_TOKENS_FILE
        runtime_lookup_data.DEFAULT_TOKENS_FILE = default_tokens
        try:
            result = self.lookup.run(
                ["alice"],
                variables={},
            ...
            runtime_lookup_data.DEFAULT_TOKENS_FILE = previous_default
