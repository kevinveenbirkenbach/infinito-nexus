...
class TestApplicationsIfGroupAndDeps(unittest.TestCase):
    ...
    @patch("plugins.filter.applications_if_group_and_deps.yaml.safe_load")
    @patch("plugins.filter.applications_if_group_and_deps.open", new_callable=mock_open)
    @patch("plugins.filter.applications_if_group_and_deps.os.path.isfile")
