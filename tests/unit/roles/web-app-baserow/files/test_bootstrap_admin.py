...
def _fake_django_modules(user_cls: MagicMock) -> dict[str, ModuleType]:
...
class TestBootstrapAdmin(unittest.TestCase):
    def _run(
        ...
        with (
            ...
            patch.dict(sys.modules, _fake_django_modules(user_cls)),
            redirect_stdout(buf),
        ):
            runpy.run_path(str(SCRIPT_PATH), run_name="__main__")

        return buf.getvalue().strip(), user_instance, manager

    def test_creates_new_superuser(self) -> None:
        out, user, manager = self._run(created=True)
    ...
    def test_skips_password_when_user_exists(self) -> None:
        out, user, manager = self._run(created=False)
