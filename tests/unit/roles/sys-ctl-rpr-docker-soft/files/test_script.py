...
def load_script_module():
    ...
    repo_root = test_file.parents[
        5
    ]  # .../tests/unit/roles/sys-ctl-rpr-docker-soft/files -> repo root
    script_path = (
...
class TestRepairDockerSoft(unittest.TestCase):
    ...
    def test_main_restarts_and_counts_errors_and_envfile_usage(self):
        ...
        try:
            ...
            restart_cmds = [
                c for c in cmd_log if " docker-compose" in c and " restart" in c
            ]
            self.assertTrue(
                any(
                    'cd "/BASE/app1"' in c
                    and '--env-file "/BASE/app1/.env/env"' in c
                    and '-p "app1"' in c
                    and " restart" in c
