...
class TestGetAppConf(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.applications = {
            "myapp": {
                ...
                "docker": {
                    "images": {"myapp": "repo/myapp"},
                    "versions": {"myapp": "1.2.3"},
                },
            }
        }
    ...
    def test_docker_image_strict_true_positive(self):
        ...
        result = get_app_conf(
            self.applications, "myapp", "docker.images.myapp", strict=True
        )
    ...
    def test_docker_image_strict_true_negative(self):
        ...
        with self.assertRaises(AnsibleFilterError) as ctx:
            get_app_conf(
                self.applications, "myapp", "docker.images.unknown", strict=True
            )
    ...
    def test_docker_image_strict_false_positive(self):
        ...
        result = get_app_conf(
            self.applications, "myapp", "docker.images.myapp", strict=False
        )
        self.assertEqual(result, "repo/myapp")
    ...
    def test_docker_image_strict_false_negative(self):
        ...
        result = get_app_conf(
            self.applications, "myapp", "docker.images.unknown", strict=False
        )
        self.assertFalse(result)

    def test_list_indexing_positive(self):
        """Test access of list index, present."""
        apps = {"app": {"foo": [{"bar": "x"}, {"bar": "y"}]}}
    ...
    def test_default_none_non_strict(self):
        ...
        result = get_app_conf(
            self.applications, "myapp", "features.baz", strict=False, default=None
        )
        self.assertFalse(result)
    ...
    def test_list_indexing_negative_with_default(self):
        ...
        apps = {"app": {"foo": [{"bar": "x"}]}}
        result = get_app_conf(
            apps, "app", "foo[1].bar", strict=False, default="fallback"
        )
