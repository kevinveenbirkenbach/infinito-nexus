...
class TestKeycloakImportFlowSpot(unittest.TestCase):
    def test_keycloak_service_does_not_use_startup_import_flag(self):
        content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/templates/compose.yml.j2"))
        )
    ...
    def test_keycloak_has_one_shot_realm_import_task(self):
        entry_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/tasks/05_realm_import.yml"))
        )
        run_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/tasks/05a_realm_import_run.yml"))
        )
    ...
    def test_keycloak_resolves_realm_dictionary_at_task_runtime(self):
        content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/tasks/03_init.yml"))
        )
    ...
    def test_keycloak_vars_use_runtime_application_lookup_for_redirects(self):
        content = read_text(
            str(pathlib.Path(f"roles/web-app-keycloak/{ROLE_FILE_VARS_MAIN}"))
        )
    ...
    def test_keycloak_provisions_realm_users_without_ldap(self):
        core_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/tasks/01_core.yml"))
        )
        user_sync_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/tasks/update/08_users_realm.yml"))
        )
        script_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/files/ensure_realm_user.sh"))
        )
    ...
    def test_keycloak_only_imports_and_keeps_ldap_federation_when_ldap_is_enabled(self):
        ...
        import_content = read_text(
            str(pathlib.Path("roles/web-app-keycloak/templates/import/realm.json.j2"))
        )
        cleanup_content = read_text(
            str(
                pathlib.Path("roles/web-app-keycloak/tasks/update/05_ldap_disabled.yml")
            )
        )
