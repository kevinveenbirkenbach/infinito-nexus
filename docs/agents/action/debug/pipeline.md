...
- If multiple roles share the same root cause, you MUST first deploy them together via the `INFINITO_APPS="<role-a> <role-b>"` form described in [Role Loop](../iteration/role.md), then run the alphabetically first role of the batch individually as the exemplary single-app deploy.
- You MUST NOT pass more than 7 roles to a single `INFINITO_APPS=...` invocation, regardless of how many roles share the same root cause. If more than 7 roles share a root cause, you MUST split them into batches of at most 7. Within each batch, list roles in ascending alphabetical order. Process batches in ascending alphabetical order of their first role, sequentially. Once the shared root cause is verified fixed in one batch, you MUST skip all remaining batches that share that same root cause.
- After each grouped-INFINITO_APPS deploy plus its exemplary single-app deploy, you MUST ask the operator to commit the fix per [commit.md](../commit.md).
