...
- First contact with a previously-untouched `VARIANT=<idx>` MUST be `make deploy-fresh-purged-apps APPS=<roles> VARIANT=<idx>`, never a reuse target. Reuse re-pins the live stack onto stale volumes, DB rows and network aliases from the previously-pinned variant, producing split-brain app state.
...
- To run a one-off sidecar image against the same docker daemon (e.g. a Playwright runner with a patched `.env`), use `make run IMAGE=<ref> [CMD=...] [RUN_FLAGS=...]`.
