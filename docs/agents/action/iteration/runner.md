...
- The systemd services installed by `svc.sh` require a host with a real init system. Registration, service install, and service start are skipped when `DOCKER_IN_CONTAINER=true`; the role can be fully deployed and tested in DinD (binary extraction, `.env` file, and the end-to-end app deploy via `test.sh` all work). `test.env.j2` renders `INFINITO_DISTRO` from `ansible_distribution | lower` so the nested `deploy-fresh-purged-apps` call uses the same image as the outer DinD (e.g. centos inside a centos container, not the `defaults.sh` fallback of debian).
