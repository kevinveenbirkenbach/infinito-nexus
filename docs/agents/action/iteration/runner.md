...
- The systemd services installed by `svc.sh` require a host with a real init system. Registration, service install, and service start are skipped when `DOCKER_IN_CONTAINER=true`; the role can be fully deployed and tested in DinD (binary extraction, `.env` file, and the end-to-end app deploy via `test.sh` all work). `test.sh` forces `INFINITO_DISTRO=debian` for the nested `deploy-fresh-purged-apps` call regardless of the runner host distro — the CI package-cache proxy only covers apt/pacman repos, so all distros (including centos) run the nested deploy against a debian image.
