...
per-application OU (`ou=<application_id>,ou=roles,...`) and adds an
opt-in tenancy layer on top that expands per tenant identifier for
apps whose capability model is per-tenant. WordPress is the first
tenant-aware consumer; other roles migrate to the new layout without
gaining per-tenant expansion.

## Acceptance Criteria

### Schema extension in role config

- [ ] A new optional key `rbac.tenancy` MUST be supported in
...
- [ ] When `rbac.tenancy.axis == "none"` or the `rbac.tenancy` block is
...
- [ ] When `rbac.tenancy.axis == "domain"`, the pipeline MUST read the
...
- [ ] Tenant identifiers MUST always be DNS domains drawn from
  ...
  cannot drift.
- [ ] Within a tenant-aware role, individual `rbac.roles.<role_name>`
...
- [ ] Every role that declares an `rbac:` block MUST be provisioned
  under a per-application OU, regardless of whether it is tenant-aware.
  The DN patterns are:

  ```
  # role in a non-tenant app (tenancy.axis == "none" or absent)
  cn=<role_name>,ou=<application_id>,ou=roles,<LDAP_DN_BASE>

  # role in a tenant-aware app, per tenant (scope: "per_tenant", default)
  cn=<role_name>,ou=<tenant_id>,ou=<application_id>,ou=roles,<LDAP_DN_BASE>
  ...
  cn=administrator,ou=web-app-nextcloud,ou=roles,dc=infinito,dc=example

  # tenant-aware app: WordPress Multisite with canonical domains
  # ["blog.example", "shop.example"]
  cn=editor,ou=blog.example,ou=web-app-wordpress,ou=roles,dc=infinito,dc=example
  cn=subscriber,ou=blog.example,ou=web-app-wordpress,ou=roles,dc=infinito,dc=example
  cn=editor,ou=shop.example,ou=web-app-wordpress,ou=roles,dc=infinito,dc=example
  cn=network-administrator,ou=web-app-wordpress,ou=roles,dc=infinito,dc=example
  ```

- [ ] The flat pattern from
  [004-generic-rbac-ldap-auto-provisioning.md](004-generic-rbac-ldap-auto-provisioning.md)
...
- [ ] The missing intermediate OUs (`ou=<application_id>`, and for
  tenant-aware apps `ou=<tenant_id>`) MUST be auto-created by the
  provisioning pipeline so that an operator does not need a separate
  step to prepare the OU tree.
- [ ] The gate from
  [004-generic-rbac-ldap-auto-provisioning.md](004-generic-rbac-ldap-auto-provisioning.md)
...
- [ ] The implicit `administrator` group from
  [004-generic-rbac-ldap-auto-provisioning.md](004-generic-rbac-ldap-auto-provisioning.md)
  MUST continue to be auto-added for every deployed role with an
  `rbac:` block. In the new layout it lands at
  `cn=administrator,ou=<application_id>,ou=roles,...` for non-tenant
  roles, and per tenant under `ou=<tenant_id>,ou=<application_id>,ou=roles,...`
...
- [ ] A new Ansible lookup plugin MUST be added at
...
- [ ] The plugin MUST read `RBAC.GROUP.NAME` from the
...
- [ ] The plugin MUST consult the target application's `rbac` config
...
- [ ] Every existing callsite that today uses the pattern
...
- [ ] [tests/integration/lookups/test_usage.py](../../tests/integration/lookups/test_usage.py),
...
- [ ] The Keycloak LDAP group federation mapper MUST be reconfigured
...
- [ ] The OIDC `groups` claim with `full.path=true` MUST emit paths
...
- [ ] The mu-plugin MUST detect Multisite at runtime (`is_multisite()`).
...
- [ ] For each site in the network (`get_sites()`), the mu-plugin MUST
  ...
  that site's tenant.
- [ ] When the user has at least one `/roles/web-app-wordpress/<tenant_id>/<role_name>`
  entry for a site and is not yet a registered member of that site,
  the mu-plugin MUST call `add_user_to_blog($blog_id, $user_id,
  $role)`.
- [ ] When the user has no matching claim entry for a site and was
...
- [ ] Running the OIDC login flow multiple times in a row MUST
...
- [ ] The provisioning pipeline MUST NOT remove DNs that are still
...
- [ ] A migration task MUST be added to
...
- [ ] The migration task MUST be idempotent: running it a second time
...
- [ ] Immediately after the DN rewrite, the migration MUST force-
...
- [ ] Consumer-side impact MUST be surveyed and updated in the same
...
- [ ] The existing Playwright suite in
...
- [ ] A new Multisite Playwright spec MUST exercise a three-site
  ...
  - Revocation: remove the network-administrator group in Keycloak
    ...
    every site.
- [ ] The Multisite spec MUST follow the same idempotency contract
  as the spec from
  [004-generic-rbac-ldap-auto-provisioning.md](004-generic-rbac-ldap-auto-provisioning.md):
...
- [ ] A unit-level test in
  ...
  each produces.
- [ ] A unit-level test MUST cover the migration task on a synthetic
...
- [ ] The migration described under "Migration from the flat layout"
...
- [ ] Consumer apps whose code hard-codes the flat
...
- [ ] A new page `docs/contributing/design/iam/rbac.md` MUST be created
...
- [ ] A new page `docs/administration/configuration/rbac.md` MUST be
...
- [ ] [mu-plugins/README.md](../../roles/web-app-wordpress/files/mu-plugins/README.md)
...
- [ ] The `web-app-wordpress` role `README.md` MUST gain a section
  ...
  inlining them.
- [ ] The rules for the new `ou=<application_id>,ou=roles,...` layout
