...
- [x] Implementation and validation for `web-app-nextcloud` follow the local iteration loop from [Role Loop](../agents/action/iteration/role.md), starting with `make deploy-fresh-purged-apps APPS=web-app-nextcloud` for the baseline and continuing with `make deploy-reuse-kept-apps APPS=web-app-nextcloud` for normal follow-up iterations.
