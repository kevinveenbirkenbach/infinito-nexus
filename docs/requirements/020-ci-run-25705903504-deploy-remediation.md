...
| **C-new-nextcloud-ldap-warm** | nextcloud user_ldap warm-cache PHP file missing | `web-app-nextcloud/tasks/plugins/user_ldap.yml:69` fails with `Fatal error: Uncaught Error: Failed opening required '/var/www/html/lib/versioncheck.php'`. Related to upstream Nextcloud image bump (see issue #227). | Pin the Nextcloud image to a tag whose layout still ships `lib/versioncheck.php`, or rebase the role's plugin path. |
...
| `web-app-nextcloud` | 27 | C-new-nextcloud-ldap-warm | — | ❌ | ⏳ | ⏳ | CI run 25890032686 — **C-new-nextcloud-ldap-warm**: `web-app-nextcloud/tasks/plugins/user_ldap.yml:69` (Warm LDAP cache) fails with `Fatal error: Uncaught Error: Failed opening required '/var/www/html/lib/versioncheck.php'`. Related to upstream Nextcloud image bump — see issue #227. Prior C14 env-flake interpretation no longer applies; this is a real regression. |
...
| ~~`web-app-listmonk`~~ | 22 | — | — | ✅ | ✅ |  | CI run 25890032686 — deploy + Playwright PASS for all declared variants (v1 verified; C11 DB-upgrade race did not reproduce). |
| `web-app-gitea` | 22 | C12 | — | ❌ | ⏳ | ⏳ | CI run 25890032686 — **C12 compose-up recurrence**: `sys-svc-compose/handlers/main.yml:76` (`compose up`) exits non-zero again after gitea image pull. Same orphan-default-network signature; the prior commit `c6affc96f` purge primitive was partially reverted by `602c05ed3 Reverted matrix fix`. Re-apply `scripts/container/purge/entity/network.sh` + global `docker network prune -f`. Playwright issues stay gated behind compose-up. |
...
| `web-app-openproject` | 20 | C9-OOM | — | ❌ | ⏳ | ⏳ | CI run 25890032686 — OOM still reproducing: `01_settings.yml:15` still exits `rc=137` after 30 retries despite `bdd59b9db` bumping `web.mem_limit` 4g → 6g. Consider a further bump to 8g and/or breaking the migration into smaller transactions. |
| `web-app-mediawiki` | 20 | C-new-mediawiki-mariadb-missing | — | ❌ | ⏳ |  | CI run 25890032686 — **C-new-mediawiki-mariadb-missing**: `web-app-mediawiki/tasks/main.yml:25` (Wait until db container healthy) loops 60× because `docker container inspect mariadb` returns no entry — mariadb container literally absent. Sibling of C12 (compose-up failed earlier). |
...
**Newly red** (was closed in the matrix above, now re-opened): `web-app-nextcloud` — new cluster `C-new-nextcloud-ldap-warm` (Nextcloud image bump, see issue #227). The prior C14 env-flake interpretation no longer applies.
