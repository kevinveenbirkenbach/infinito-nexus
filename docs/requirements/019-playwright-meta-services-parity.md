...
| `web-app-matrix` | 21 | ✅ | ✅ | ❌ | ⏳ |  | CI run 25774452286 — **C12 matrix compose-up** had the same root cause as gitea's network-label failure: an orphan `matrix` Docker network without `com.docker.compose.network=default` survived the inter-round purge. Resolved by the new orphan-default-network purge primitive `scripts/container/purge/entity/network.sh` + global `docker network prune -f` in `scripts/container/purge/apps.sh` (commit `c6affc96f`). Local v0 deploy compose-up grün; Playwright DM-scenario `matrix DM › administrator and biber can exchange a direct message in element` still times out after 4.2 min — separate matrix-spec issue. |
