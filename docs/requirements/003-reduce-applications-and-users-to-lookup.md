...
- **Batching**: all five target apps are deployed together via `make deploy-fresh-purged-apps INFINITO_APPS="web-app-keycloak web-app-dashboard web-app-matomo web-app-openwebui web-app-discourse" INFINITO_FULL_CYCLE=true` as the baseline. Inner spec-only iteration uses [rerun-spec.sh](../../scripts/tests/e2e/rerun-spec.sh). Role-asset changes trigger `make deploy-reuse-kept-apps INFINITO_APPS=<role>`. Final confirmation for each role uses `make deploy-fresh-purged-apps INFINITO_APPS=<role>`.
