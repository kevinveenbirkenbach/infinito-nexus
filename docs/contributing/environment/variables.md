...
| `INFINITO_REGISTRY_CACHE_HOST_PATH` | Host-side path bind-mounted into the always-on `registry-cache` service for blob/manifest persistence. | `/tmp/infinito/core/registry-cache/mirror` | [compose.yml](../../../compose.yml) |
| `INFINITO_REGISTRY_CACHE_CA_HOST_PATH` | Host-side path holding the registry-cache MITM CA bundle. Bind-mounted into `registry-cache` (writable) and into `infinito` (read-only) so its dockerd can install the CA via [registry-cache-ca.sh](../../../scripts/docker/registry-cache-ca.sh) before pulling. | `/tmp/infinito/core/registry-cache/ca` | [compose.yml](../../../compose.yml) |
| `INFINITO_REGISTRY_CACHE_MAX_SIZE` | Maximum on-disk size of the rpardini/docker-registry-proxy cache. | `32g` | [compose.yml](../../../compose.yml) |
