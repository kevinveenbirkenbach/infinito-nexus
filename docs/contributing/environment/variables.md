...
| `INFINITO_REGISTRY_CACHE_HOST_PATH` | Host-side path bind-mounted into the `registry-cache` service for blob/manifest persistence. The service is gated by the `cache` compose profile (active locally, inactive in CI; see [profile.py](../../../cli/deploy/development/profile.py)). Mandatory in `compose.yml`; default supplied by [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh). | `/tmp/infinito/core/registry-cache/mirror` | [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh), [compose.yml](../../../compose.yml) |
| `INFINITO_REGISTRY_CACHE_CA_HOST_PATH` | Host-side path holding the registry-cache MITM CA bundle. Bind-mounted into `registry-cache` (writable) and into `infinito` (read-only) so its dockerd can install the CA via [registry-cache-ca.sh](../../../scripts/docker/registry-cache-ca.sh) before pulling. Mandatory in `compose.yml`; default supplied by [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh). | `/tmp/infinito/core/registry-cache/ca` | [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh), [compose.yml](../../../compose.yml) |
| `INFINITO_REGISTRY_CACHE_MAX_SIZE` | Maximum on-disk size of the rpardini/docker-registry-proxy cache. Mandatory in `compose.yml`; [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh) computes the default as **half the free disk space** at `INFINITO_REGISTRY_CACHE_HOST_PATH` (or its closest existing ancestor) so the cache adapts to the host without starving the rest of the system. | Half of `df --output=avail` at the cache path, minimum `1g`, fallback `2g` if `df` fails. | [registry_cache.sh](../../../scripts/meta/env/registry_cache.sh), [compose.yml](../../../compose.yml) |
