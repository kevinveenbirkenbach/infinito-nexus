...
| `SERVICES_DISABLED` | Space- or comma-separated list of compose service names to disable after inventory creation. Sets `enabled: false` and `shared: false` for every matching service across all applications. Use this to reduce resource usage on low-hardware machines. Example: `SERVICES_DISABLED="oidc ldap matomo"`. | Empty by default (no services disabled). | [services_disabler.py](../../../cli/administration/inventory/provision/services_disabler.py) |
...
| `INFINITO_GHCR_MIRROR_PREFIX` | Prefix used when resolving GHCR mirror image names. | `mirror` | [github.sh](../../../scripts/meta/env/github.sh), [mirrors.py](../../../cli/administration/deploy/development/mirrors.py) |
| `INFINITO_IMAGE` | Full container image reference used by Compose and deploy tooling. | `infinito-${INFINITO_DISTRO}` when unset. | [github.sh](../../../scripts/meta/env/github.sh), [compose.yml](../../../compose.yml), [compose.py](../../../cli/administration/deploy/development/compose.py) |
...
| `INFINITO_REGISTRY_CACHE_*`, `INFINITO_PACKAGE_CACHE_*` | Cache-stack variables (registry-cache, package-cache, package-cache-frontend). For purpose, defaults, and consumers, see [cache.md](cache.md). | see [cache.md](cache.md) | [registry.sh](../../../scripts/meta/env/cache/registry.sh), [package.sh](../../../scripts/meta/env/cache/package.sh) |
...
| `INFINITO_WAIT_HEALTH_TIMEOUT_S` | Timeout while waiting for the `infinito` container to become healthy. | `600` seconds. | [compose.py](../../../cli/administration/deploy/development/compose.py) |
