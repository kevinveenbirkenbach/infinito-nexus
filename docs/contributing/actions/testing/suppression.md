...
| `shared`            | comment block above | [test_service_shared_consistency.py](../../../../tests/lint/ansible/services/test_service_shared_consistency.py)                     | Marks a service whose `enabled: true` legitimately does not require `shared: true`.                              |
...
| `self-path-reference`| same line          | [tests/lint/repository/test_no_self_path_reference.py](../../../../tests/lint/repository/test_no_self_path_reference.py) | Allows a file to contain its own relative path from the repo root as a string literal on the marked line. Reserved for cases where the self-reference is load-bearing (e.g. a static site generator that injects its own source path into the rendered output). Files at the repo root (no `/` in their relative path) are exempt without a marker. |
| `role-file-spot`    | same or above       | [tests/lint/ansible/roles/test_file_spot.py](../../../../tests/lint/ansible/roles/test_file_spot.py) | Allows a `.py` file to carry a literal role-file path (e.g. `"meta/services.yml"`) on the marked line instead of the matching `ROLE_FILE_*` constant from `utils.roles.mapping`. Reserved for tests that assert the constant's literal value, migration scripts that operate on raw paths before the SPOT applies, or doc strings that embed the path. |
| `playwright-service-flag` | comment block above | [tests/integration/roles/test_playwright_env_services_match.py](../../../../tests/integration/roles/test_playwright_env_services_match.py) | Exempts a top-level service in `roles/<role>/meta/services.yml` from the "must be registered as `<NAME>_SERVICE_ENABLED=` in `templates/playwright.env.j2`" check. Use only when the service legitimately cannot or must not be gated from Playwright (e.g. requirement-006 self-gate exceptions where the role IS the provider, like `web-app-keycloak` for `oidc`). |
| `playwright-service-gate` | same or above   | [tests/integration/roles/test_playwright_spec_env_gates.py](../../../../tests/integration/roles/test_playwright_spec_env_gates.py) | Exempts a `<NAME>_SERVICE_ENABLED=` line in `roles/<role>/templates/playwright.env.j2` from the "must be gated by `requireService` / `skipUnlessServiceEnabled` / `isServiceEnabled` / `isServiceDisabledReason` in `files/playwright.spec.js`" check. Use only when the flag legitimately exists for non-spec consumers but the spec has no scenario to gate. |
...
Use `cache-read` only when the file legitimately does not belong to the project-tree LRU cache, e.g. a tempdir written and read inside the same test, a file outside the repository root, or an in-memory `io.StringIO`-backed `Path`. Reads of project files (under `roles/`, `tasks/`,
