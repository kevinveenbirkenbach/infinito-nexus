...
| `role-file-spot`    | same or above       | [tests/lint/repository/test_role_file_spot.py](../../../../tests/lint/repository/test_role_file_spot.py) | Allows a `.py` file to carry a literal role-file path (e.g. `"meta/services.yml"`) on the marked line instead of the matching `ROLE_FILE_*` constant from `utils.roles.mapping`. Reserved for tests that assert the constant's literal value, migration scripts that operate on raw paths before the SPOT applies, or doc strings that embed the path. |
