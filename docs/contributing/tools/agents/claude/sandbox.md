...
|---|---|---|
| `allowedDomains` | Explicit list of every host the agent or its tooling needs to reach (PyPI + pythonhosted, Ansible Galaxy, GitHub + raw/api/codeload + GHCR, npm registry, Let's Encrypt, hCaptcha, gstatic, the project's own `*.infinito.nexus` and `infinito.example` hosts, and the documentation/community sites referenced from research workflows). | Wildcards (`*`) are **not** honored by this gate — sandboxed network access is enforced per-host, and any unlisted host triggers an interactive `SandboxNetworkAccess` prompt. Add new hosts here when introducing tooling that fetches from a new origin; egress is still bounded by any host-level firewall on top. |
