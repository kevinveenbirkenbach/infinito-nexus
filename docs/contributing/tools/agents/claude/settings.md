...
| `tee /tmp/*` | Mirroring a long-running command's combined output to a log file under `/tmp/` for later inspection (`… 2>&1 \| tee /tmp/<name>.log`). | Pipelines (`\|`) and redirects (`2>&1`) regularly cause the sandbox auto-allow heuristic to decline, so the right-hand side of the pipe needs its own allow entry. Restricting the path to `/tmp/*` keeps this narrow: logs land inside the sandbox's `allowWrite` scope and outside the repo tree. | Write is bounded to `/tmp/*` by the rule itself and reinforced by `sandbox.filesystem.allowWrite`. No mutation outside `/tmp`. No network. |
