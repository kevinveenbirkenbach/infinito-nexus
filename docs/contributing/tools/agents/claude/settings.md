...
| `make *` | Running any `Makefile` target (e.g. `make deploy-fresh-purged-apps`, `make up`, `make test`). | CLAUDE.md instructs the agent to prefer `make` targets over raw `docker`/`ansible-playbook`/`python` invocations. Long `make` invocations in practice combine multi-word `VAR="a b c"` arguments with trailing redirections/pipes (`2>&1 | tee /tmp/...`), and both shapes routinely fall outside the sandbox auto-allow heuristic. A single allow entry covers the whole namespace so the agent can follow the "prefer make targets" guidance without a prompt. | The Makefile is checked-in and peer-reviewed — targets themselves are trusted. Every action a target performs still runs inside the sandbox (`allowWrite` bounds writes, `allowedDomains` bounds network). Destructive `rm -rf`, `sudo`, `git reset --hard`, and `gh` deny rules still override this allow. |
...
| Permission | Reason |
|---|---|
| `for *` / `while *` / `until *` | Shell control-flow loops (`for f in ...; do ...; done`, `while read ...`, `until cond; do ...`). CLAUDE.md forbids these constructs — the documented replacement is a single `grep` with multiple file arguments, `xargs`, or a glob. Blocking at the permission layer converts the soft textual rule into a hard gate: a violating attempt fails fast, the agent gets concrete feedback, and refactors to the flat form instead of silently running a loop. The sandbox auto-allow heuristic does not recognize loop keywords anyway, so this deny has no cost vs. any alternative (allow would also require explicit entries; ask would deadlock). Subcommands inside a legitimate loop body would still be bounded by their own rules — but since loops themselves are forbidden, this path should never be reached. |
