...
| `echo*` | Emitting literal strings, typically as the right-hand side of shell fallbacks like `ls <path> 2>/dev/null \|\| echo "no dir"`. Such compound commands are split at `\|\|`, `&&`, `;`, `\|`, so each subcommand is matched separately and `echo` needs its own allow entry. | Avoids per-invocation approval prompts for common shell idioms used during inspection and probing. | Writes only to stdout; no file, network, or process side-effects on its own. Dangerous combinations (e.g. `echo ... > file`, `echo ... \| sh`) remain bounded by the sandbox `allowWrite` scope and by `deny` rules on `sudo*` and `rm -rf*`. |
