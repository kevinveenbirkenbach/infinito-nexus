...
- MUST ship `templates/playwright.env.j2` and `files/playwright.spec.js`, with at least one assertion that covers the role's canonical landing surface.
...
Release candidate.
The role meets every `beta` criterion AND has gone through at least one full FULL_CYCLE matrix run plus an external review pass without regression.
