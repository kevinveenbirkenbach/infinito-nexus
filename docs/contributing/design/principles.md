...
| Build only what is needed now. | Unused code adds maintenance burden without value. | [YAGNI](https://en.wikipedia.org/wiki/You_aren%27t_gonna_need_it) | Do not implement functionality until it is required. Avoid speculative abstractions, optional hooks, and future-proofing that no current requirement justifies. |
